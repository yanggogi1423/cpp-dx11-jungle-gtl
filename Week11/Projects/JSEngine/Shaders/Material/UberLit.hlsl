...
struct PSInput
{
    ...
    float2 UV : TEXCOORD2;
    float BoneHeatmapWeight : TEXCOORD6;
...
void ApplySkeletalSkinning(SkeletalVSInput input, out float3 outPosition, out float3 outNormal, out float4 outTangent)
{
    ...
    for (int j = 0; j < 4; ++j)
    {
        ...
        float4x4 skinMatrix = BoneMatrices[boneIndex].Mat;

        skinnedPosition += mul(float4(input.Position, 1.0f), skinMatrix).xyz * weight;
        skinnedNormal += mul(input.Normal, (float3x3) skinMatrix) * weight;
        skinnedTangent += mul(input.Tangent.xyz, (float3x3) skinMatrix) * weight;
    }

    float normalLenSq = dot(skinnedNormal, skinnedNormal);
    outNormal = normalLenSq > 1e-8f ? skinnedNormal * rsqrt(normalLenSq) : normalize(input.Normal);

    float tangentLenSq = dot(skinnedTangent, skinnedTangent);
    float3 tangent = tangentLenSq > 1e-8f ? skinnedTangent * rsqrt(tangentLenSq) : normalize(input.Tangent.xyz);

    outPosition = skinnedPosition;
    outTangent = float4(tangent, input.Tangent.w);
}
...
PSInput SkeletalMeshVS(SkeletalVSInput input)
{
    ...
    ApplySkeletalSkinning(input, skinnedPosition, skinnedNormal, skinnedTangent);

    VSInput passThrough;
    passThrough.Position = skinnedPosition;
    passThrough.Color = input.Color;
    passThrough.Normal = skinnedNormal;
    passThrough.UV = input.UV;
    passThrough.Tangent = skinnedTangent;
    PSInput output = mainVS(passThrough);

    const int selectedBoneIndex = (int)BoneWeightHeatmapBoneIndex;
    ...
    if (selectedBoneIndex >= 0)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            if ((int)input.BoneIndices[i] == selectedBoneIndex)
            {
    ...
    output.BoneHeatmapWeight = saturate(selectedWeight);
    return output;
}
...
float3 GetHeatmapColor(float weight)
{
    ...
    // Persona-like bone weight ramp: dark blue for no influence, then blue/cyan/green/yellow/orange/red.
    const float3 C0 = float3(0.0f, 0.0f, 0.35f);
    const float3 C1 = float3(0.0f, 0.0f, 1.0f);
    const float3 C2 = float3(0.0f, 0.85f, 1.0f);
    const float3 C3 = float3(0.0f, 1.0f, 0.0f);
    const float3 C4 = float3(1.0f, 1.0f, 0.0f);
    const float3 C5 = float3(1.0f, 0.45f, 0.0f);
    const float3 C6 = float3(1.0f, 0.0f, 0.0f);

    if (weight < 0.10f)
    {
        return lerp(C0, C1, weight / 0.10f);
    }
    if (weight < 0.25f)
    {
        return lerp(C1, C2, (weight - 0.10f) / 0.15f);
    }
    if (weight < 0.45f)
    {
        return lerp(C2, C3, (weight - 0.25f) / 0.20f);
    }
    if (weight < 0.70f)
    {
        return lerp(C3, C4, (weight - 0.45f) / 0.25f);
    }
    if (weight < 0.90f)
    {
        return lerp(C4, C5, (weight - 0.70f) / 0.20f);
    }

    return lerp(C5, C6, (weight - 0.90f) / 0.10f);
}
...
PSOutput mainPS(PSInput input) : SV_TARGET
{
    ...
    if (bBoneWeightHeatmap > 0.5f)
    {
        const float weight = saturate(input.BoneHeatmapWeight);
        const float3 heatmapColor = GetHeatmapColor(weight);
