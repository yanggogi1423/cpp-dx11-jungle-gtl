...
void UObjViewerEngine::Init(FWindowsWindow* InWindow)
{
	...
	Progress.Report("Starting object viewer...", 0.04f);
	...
	Progress.Report("Loading viewer settings...", 0.72f);
	FObjViewerSettings::Get().LoadFromFile(FObjViewerSettings::GetDefaultSettingsPath());
	
	Progress.Report("Creating viewer UI...", 0.78f);
	MainPanel.Create(Window, Renderer, this);

	// 프리뷰 전용 월드 생성
	Progress.Report("Creating preview world...", 0.84f);
	if (WorldList.empty())
	{
	...
	// 뷰포트 및 카메라 설정
	Progress.Report("Preparing viewer viewport...", 0.92f);
	...
	Progress.Report("Object viewer ready.", 1.0f);
}
