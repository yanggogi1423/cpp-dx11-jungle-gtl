...
DEFINE_CLASS(URuntimeUILayoutAsset, UObject)
REGISTER_FACTORY(URuntimeUILayoutAsset)
...
bool URuntimeUILayoutAsset::SaveToFile(const FString& Path)
{
	...
	MetaData.ClassName = URuntimeUILayoutAsset::StaticClass()->ClassName;
	...
	const bool bSaved = FAssetFile::Save(NormalizedPath, MetaData, [this](FArchive& Ar)
	{
		Serialize(Ar);
		return true;
	});
...
bool URuntimeUILayoutAsset::LoadFromFile(const FString& Path)
{
	...
	{
		if (MetaData.ClassName != URuntimeUILayoutAsset::StaticClass()->ClassName)
		{
			return false;
		}
		GRuntimeUILayoutSerializePayloadVersion = MetaData.PayloadVersion;
		Serialize(Ar);
		return true;
	});
	GRuntimeUILayoutSerializePayloadVersion = PreviousPayloadVersion;

	if (!bLoaded || MetaData.ClassName != URuntimeUILayoutAsset::StaticClass()->ClassName)
	{
		return false;
	}
