...
FArchive& FJsonReader::operator<<(FRotator& Value)
{
	json::JSON& Current = *ScopeStack.back();
	if (Current.JSONType() == json::JSON::Class::Array)
	{
		...
		if (CurrentIdx < (int32)Current.length())
		{
			auto& RotArr = Current[CurrentIdx++];
			if (RotArr.length() >= 3)
			{
				Value.Pitch = static_cast<float>(RotArr[0].ToFloat());
				Value.Yaw = static_cast<float>(RotArr[1].ToFloat());
				Value.Roll = static_cast<float>(RotArr[2].ToFloat());
			}
		}
	}
	else if (!CurrentKey.empty())
	{
		auto& RotArr = Current[CurrentKey.c_str()];
		if (RotArr.length() >= 3)
		{
			Value.Pitch = static_cast<float>(RotArr[0].ToFloat());
			Value.Yaw = static_cast<float>(RotArr[1].ToFloat());
...
FArchive& FJsonReader::operator<<(FQuat& Value)
{
	json::JSON& Current = *ScopeStack.back();
	if (Current.JSONType() == json::JSON::Class::Array)
	{
		...
		if (CurrentIdx < (int32)Current.length())
		{
			auto& QuatArr = Current[CurrentIdx++];
			if (QuatArr.length() >= 4)
			{
				Value.X = static_cast<float>(QuatArr[0].ToFloat());
				Value.Y = static_cast<float>(QuatArr[1].ToFloat());
				Value.Z = static_cast<float>(QuatArr[2].ToFloat());
				Value.W = static_cast<float>(QuatArr[3].ToFloat());
			}
		}
	}
	else if (!CurrentKey.empty())
	{
		auto& QuatArr = Current[CurrentKey.c_str()];
		if (QuatArr.length() >= 4)
		{
			Value.X = static_cast<float>(QuatArr[0].ToFloat());
			Value.Y = static_cast<float>(QuatArr[1].ToFloat());
