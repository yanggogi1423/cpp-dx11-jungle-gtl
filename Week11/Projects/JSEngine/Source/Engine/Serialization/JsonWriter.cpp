...
FArchive& FJsonWriter::operator<<(FRotator& Value)
{
	if (ScopeStack.back()->JSONType() == json::JSON::Class::Array)
	{
		json::JSON RotArray = json::Array(Value.Pitch, Value.Yaw, Value.Roll);
		ScopeStack.back()->append(RotArray);
	}
	else if (!CurrentKey.empty())
	{
		json::JSON RotArray = json::Array(Value.Pitch, Value.Yaw, Value.Roll);
		(*ScopeStack.back())[CurrentKey] = RotArray;
		CurrentKey.clear();
	}
	return *this;
}

FArchive& FJsonWriter::operator<<(FQuat& Value)
{
	if (ScopeStack.back()->JSONType() == json::JSON::Class::Array)
	{
		json::JSON QuatArray = json::Array(Value.X, Value.Y, Value.Z, Value.W);
		ScopeStack.back()->append(QuatArray);
	}
	else if (!CurrentKey.empty())
	{
		json::JSON QuatArray = json::Array(Value.X, Value.Y, Value.Z, Value.W);
		(*ScopeStack.back())[CurrentKey] = QuatArray;
		CurrentKey.clear();
	}
	return *this;
}
