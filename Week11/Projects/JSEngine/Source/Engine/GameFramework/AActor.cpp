...
void AActor::TestOwnerFunction()
{
	UE_LOG("Blueprint target owner");
}
...
FString AActor::MakeUniqueComponentName(const UActorComponent* TargetComponent, const FString& RequestedName, bool bAlwaysAppendNumber) const
{
    ...
    if (BaseName.empty())
    {
        BaseName = TargetComponent && TargetComponent->GetClass() ? TargetComponent->GetClass()->ClassName : "UActorComponent";
    }
...
void AActor::SyncActorTransformProperties()
{
...
void AActor::PostEditProperty(const char* PropertyName)
{
    ...
    if (Name == "PendingActorLocation")
    {
        SetActorLocation(PendingActorLocation);
    }
    else if (Name == "PendingActorRotation")
    {
        SetActorRotation(PendingActorRotation);
    }
    else if (Name == "PendingActorScale")
    {
...
UActorComponent* AActor::AddComponentByClass(const UClass* Class)
{
    ...
    UObject* Obj = FObjectFactory::Get().Create(Class->ClassName.c_str());
    if (!Obj)
        return nullptr;
    ...
    Comp->SetOwner(this);
    Comp->SetFName(FName(MakeUniqueComponentName(Comp, Class->ClassName.c_str(), true)));
