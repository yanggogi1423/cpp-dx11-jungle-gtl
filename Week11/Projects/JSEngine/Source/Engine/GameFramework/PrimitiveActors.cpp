...
{
    InitDefaultComponents(AddComponent<USkeletalMeshComponent>());
}

void ASkeletalMeshActor::InitDefaultComponents(USkeletalMeshComponent* InSkeletalMeshComp)
{
    SkeletalMeshComp = InSkeletalMeshComp ? InSkeletalMeshComp : AddComponent<USkeletalMeshComponent>();
...
void ADecalSpotLightActor::InitDefaultComponents()
{
    ...
    if (DecalMat == nullptr)
    {
        UMaterial* DecalOriginMat = Cast<UMaterial>(FResourceManager::Get().GetMaterialInterface("Asset/Material/DecalMat.uasset"));
        if (DecalOriginMat)
        {
