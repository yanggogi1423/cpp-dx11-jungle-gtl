...
APawn* AGameModeBase::SpawnDefaultPawn()
{
    ...
    if (!DefaultPawnPrefabPath.empty())
    {
        ...
        if (APawn* PrefabPawn = Cast<APawn>(PrefabActor))
        {
            ...
            ApplyPlayerStartTransform(DefaultPawn, SpawnLocation, SpawnRotation, true);
...
void AGameModeBase::ApplyPlayerStartTransform(
    APawn* Pawn,
    const FVector& SpawnLocation,
    const FVector& SpawnRotation,
    bool bPreserveCameraSetup) const
{
    if (!Pawn)
    {
        return;
