...
void USkinnedMeshComponent::UpdateWorldAABB() const
{
	...
	const bool bGPUSkinning = FProjectRenderSettings::IsGPUSkinningEnabled();
    if (!bGPUSkinning && !SkinnedVertices.empty())
    {
...
void USkinnedMeshComponent::EnsureSkinningUpdated()
{
    ...
    const double SkinningUpdateStartSeconds = FPlatformTime::Seconds();

    UpdateCurrentGlobalPose();
    UpdateSkinningMatrices();
	if (!FProjectRenderSettings::IsGPUSkinningEnabled())
    {
        SkinVerticesCPU();
    }
    ...
    FSkinningRuntimeStatCollector::RecordUpdate(
        (FPlatformTime::Seconds() - SkinningUpdateStartSeconds) * 1000.0);
}
...
{
    const double PoseUpdateStartSeconds = FPlatformTime::Seconds();

    if (!HasValidMesh())
    {
        FSkinningRuntimeStatCollector::RecordPoseUpdate(
            (FPlatformTime::Seconds() - PoseUpdateStartSeconds) * 1000.0);
        return;
    }
...
{
    const double MatrixUpdateStartSeconds = FPlatformTime::Seconds();
    ...
    if (!HasValidMesh())
    {
        FSkinningRuntimeStatCollector::RecordMatrixUpdate(
            (FPlatformTime::Seconds() - MatrixUpdateStartSeconds) * 1000.0);
        return;
    }
...
{
    const double CPUVertexSkinningStartSeconds = FPlatformTime::Seconds();

    if (!HasValidMesh())
    {
        SkinnedVertices.clear();
        FSkinningRuntimeStatCollector::RecordCPUVertex(
            (FPlatformTime::Seconds() - CPUVertexSkinningStartSeconds) * 1000.0,
            0);
        return;
    }
    ...
    FSkinningRuntimeStatCollector::RecordCPUVertex(
        (FPlatformTime::Seconds() - CPUVertexSkinningStartSeconds) * 1000.0,
        static_cast<uint32>(SourceVertices.size()));
}
...
void USkinnedMeshComponent::RefreshSkeletalMeshState()
{
	Materials.clear();
	CurrentLocalPose.clear();
	CurrentGlobalPose.clear();
