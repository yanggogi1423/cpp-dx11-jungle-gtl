...
void UMeshComponent::Serialize(FArchive& Ar)
{
	...
	else if (Ar.IsSaving())
	{
		for (auto& Mat : Materials)
		{
			...
			{
				FString MaterialPath = FPaths::Normalize(MatInst->GetFilePath());
				if (MaterialPath.empty() && MatInst->Parent)
				{
					MaterialPath = FPaths::Normalize(MatInst->Parent->GetFilePath());
					if (MaterialPath.empty())
					{
						MaterialPath = MatInst->Parent->GetName();
					}
				}
			...
			else if (UMaterial* BaseMat = Cast<UMaterial>(Mat))
			{
				...
				const bool bFileBackedMaterial = FilePath.extension() == L".uasset";
