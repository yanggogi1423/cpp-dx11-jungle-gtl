...
{
    //	기본 도형은 Cube로 설정
    SetStaticMesh(FResourceManager::Get().LoadStaticMesh("Asset\\Mesh\\Dice\\Dice.uasset"));
}
...
void UStaticMeshComponent::PostDuplicate(UObject* Original)
{
	...
	for (int32 i = 0; i < static_cast<int32>(Orig->Materials.size()); ++i)
	{
		...
		{
			if (!OrigMatInst->GetFilePath().empty())
			{
				Materials[i] = OrigMatInst; // file-backed asset reference
			}
			else
			{
...
void UStaticMeshComponent::RefreshStaticMeshState()
{
	Materials.clear();
