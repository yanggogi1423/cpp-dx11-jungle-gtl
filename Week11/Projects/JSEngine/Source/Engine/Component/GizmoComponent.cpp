...
void UGizmoComponent::RotateTarget(float DragAmount)
{
	...
	FVector WorldRotationAxis = RotationDragAxisWorld.IsNearlyZero()
		? GetVectorForAxis(SelectedAxis)
		: RotationDragAxisWorld;
	WorldRotationAxis.NormalizeSafe();
	if (WorldRotationAxis.IsNearlyZero()) return;

	FQuat DeltaWorldQuat(WorldRotationAxis, DragAmount);
	...
	FQuat NewQuat = CurQuat * DeltaWorldQuat;
	NewQuat.Normalize();
	...
	auto ApplyRotation = [&](AActor* Actor)
		{
			...
			Actor->SetActorLocation(Pivot + DeltaWorldQuat.RotateVector(OffsetFromPivot));
			
			FQuat ActorCurQuat = FQuat::MakeFromEuler(Actor->GetActorRotation());
			FQuat ActorNewQuat = ActorCurQuat * DeltaWorldQuat;
...
{
	if (bIsFirstFrameOfDrag || RotationDragAxisWorld.IsNearlyZero())
	{
		RotationDragAxisWorld = GetVectorForAxis(SelectedAxis);
		RotationDragAxisWorld.NormalizeSafe();
	}

	FVector AxisVector = RotationDragAxisWorld;
	AxisVector.NormalizeSafe();
	if (AxisVector.IsNearlyZero()) return;
...
void UGizmoComponent::DragEnd()
{
	...
	SelectedAxis = -1;
	PendingSnapDelta = 0.0f;
	RotationDragAxisWorld = FVector::ZeroVector;
}
