...
namespace
{
	FQuat MakeCameraViewRotation(float PitchDegrees, float YawDegrees)
	{
...
bool UCameraComponent::GetEditorVisualizationDesc(FCameraEditorVisualizationDesc& OutDesc) const
{
	...
	OutDesc.LocalToComponentMatrix = FMatrix::MakeRotationX(MathUtil::PI / 2.f);
...
void UCameraComponent::LookAt(const FVector& Target)
{
	...
	SetViewWorldRotationDegrees(PitchDegrees, YawDegrees);
}
...
void UCameraComponent::SetViewWorldRotationDegrees(float PitchDegrees, float YawDegrees)
{
	SetWorldRotationQuat(MakeCameraViewRotation(PitchDegrees, YawDegrees));
}

void UCameraComponent::SetViewLocalRotationDegrees(float PitchDegrees, float YawDegrees)
{
	SetRelativeRotationQuat(MakeCameraViewRotation(PitchDegrees, YawDegrees));
}

FVector UCameraComponent::GetForwardVector() const
{
	return GetWorldQuat().GetForwardVector().GetSafeNormal();
}
...
FVector UCameraComponent::GetRightVector() const
{
	...
	if (Right.IsNearlyZero())
	{
		Right = GetWorldQuat().GetRightVector().GetSafeNormal();
	}
