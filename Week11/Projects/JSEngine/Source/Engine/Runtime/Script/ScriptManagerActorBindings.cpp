...
namespace
{
    ...
    sol::table GetComponentsByType(sol::this_state State, AActor& Actor, const FString& TypeName)
    {
        ...
        const UClass* Class = FindComponentClass(TypeName);
        if (!Class)
        {
            return Components;
        }
        ...
        for (UActorComponent* Component : Actor.GetComponents())
        {
            if (Component && Component->IsA(Class))
            {
                Components[Index++] = Component;
            }
        }
...
void FScriptManager::BindActorTypes()
{
    ...
    LUA_PROPERTY(TypeName, [](AActor& Actor) -> FString
                 { return Actor.GetClass() && !Actor.GetClass()->ClassName.empty() ? Actor.GetClass()->ClassName : ""; });
    ...
    LUA_SET(GetSkeletalMeshComponent, [](AActor& Actor) -> USkeletalMeshComponent*
            { return Cast<USkeletalMeshComponent>(GetComponentByType(Actor, "SkeletalMeshComponent")); });

    LUA_SET(GetLuaAnimInstance, [](AActor& Actor) -> ULuaAnimInstance*
            {
    USkeletalMeshComponent* Mesh = Cast<USkeletalMeshComponent>(GetComponentByType(Actor, "SkeletalMeshComponent"));
    return Mesh ? Cast<ULuaAnimInstance>(Mesh->GetAnimInstance()) : nullptr; });
    LUA_SET(GetAnimStateMachineInstance, [](AActor& Actor) -> UAnimStateMachineInstance*
            {
    USkeletalMeshComponent* Mesh = Cast<USkeletalMeshComponent>(GetComponentByType(Actor, "SkeletalMeshComponent"));
    return Mesh ? Cast<UAnimStateMachineInstance>(Mesh->GetAnimInstance()) : nullptr; });
    LUA_END_TYPE();
