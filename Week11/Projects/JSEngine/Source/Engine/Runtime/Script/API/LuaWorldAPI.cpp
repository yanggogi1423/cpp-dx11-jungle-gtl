...
namespace
{
    ...
    bool ActorMatchesClassName(const AActor* Actor, const FString& TypeName)
    {
        ...
        for (const UClass* Class = Actor->GetClass(); Class != nullptr; Class = Class->SuperClass)
        {
            if (!Class->ClassName.empty() && TypeName == Class->ClassName)
            {
                return true;
            }
        }
    ...
    void BindWorld(sol::state& Lua, sol::table& API)
    {
        ...
        World["FindActorsByType"] = [](sol::this_state State, const FString& TypeName)
        {
            ...
            if (ActiveWorld)
            {
                for (AActor* Actor : ActiveWorld->GetActors())
                {
                    if (ActorMatchesClassName(Actor, TypeName))
                    {
                        Matches.push_back(Actor);
                    }
                }
