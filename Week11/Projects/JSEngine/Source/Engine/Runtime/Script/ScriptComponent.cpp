...
namespace
{
    ...
    FString NormalizeScriptReference(const FString& ScriptName)
    {
        if (ScriptName.empty())
        {
            return {};
        ...
        {
            return FPaths::Normalize(FPaths::ToRelativeString(ScriptPath.lexically_normal().wstring()));
        }
...
void UScriptComponent::Serialize(FArchive& Ar)
{
    ...
    if (Ar.IsLoading())
    {
        ScriptName = NormalizeScriptReference(ScriptName);
...
{
    const FString NormalizedScriptName = NormalizeScriptReference(InScriptName);
    if (ScriptName == NormalizedScriptName)
    {
        if (bScriptLoaded)
        {
            HotReloadScript();
...
void UScriptComponent::PostEditProperty(const char* PropertyName)
{
    ...
    FString NewScriptName = NormalizeScriptReference(ScriptName);
