...
void FWindowsFileWatcher::SetShaderInvalidatedCallback(FShaderInvalidatedCallback InCallback)
{
	ShaderInvalidatedCallback = std::move(InCallback);
}
...
void FWindowsFileWatcher::ProcessPendingChanges()
{
	...
	for (auto It = PendingChanges.begin(); It != PendingChanges.end(); )
	{
		...
		if (std::filesystem::exists(Path) && IsRelevantShaderFile(Path))
		{
			...
			if (ShaderInvalidatedCallback)
			{
				ShaderInvalidatedCallback(RelativePath);
			}
		}
