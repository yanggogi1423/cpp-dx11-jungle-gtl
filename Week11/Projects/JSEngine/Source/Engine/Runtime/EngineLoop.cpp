...
#if WITH_EDITOR
	ShaderDirectoryWatcher.SetShaderInvalidatedCallback([](const FString& RelativePath)
		{
			if (GEngine && GEngine->IsA<UEditorEngine>())
			{
				static_cast<UEditorEngine*>(GEngine)->GetNotificationService().Info(
					FString("Shader hot reloaded: ") + RelativePath);
			}
		});
#endif
