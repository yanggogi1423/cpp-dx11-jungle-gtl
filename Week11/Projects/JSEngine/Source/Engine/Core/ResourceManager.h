...
public:
    ...
    USkeletalMesh* ImportSkeletalMeshFromSource(const FString& Path);
    void InvalidateSkeletalMesh(const FString& Path);
    ...
    // 에디터에서 socket 등 mesh data 변경 후 원본 .uasset에 저장.
    bool SaveSkeletalMesh(USkeletalMesh* Mesh);

    UAnimSequence* LoadAnimSequence(const FString& Path, const USkeletalMesh* TargetMesh, int32 AnimStackIndex = 0);
    UAnimSequence* FindAnimSequence(const FString& Path, const USkeletalMesh* TargetMesh, int32 AnimStackIndex = 0) const;
    ...
    UAnimationStateMachine* LoadAnimationStateMachine(const FString& Path);
    UAnimationStateMachine* FindAnimationStateMachine(const FString& Path) const;
    bool SaveAnimationStateMachine(const FString& Path, UAnimationStateMachine* StateMachine);
    TArray<FString> GetAnimationStateMachinePaths() const;
    ...
    std::unique_ptr<FFbxImporter> FbxImporter;

private:
    struct FFbxInspectCacheEntry
    {
        uint64 SourceWriteTimeTicks = 0;
        bool bHasMeshInfo = false;
        bool bHasAnimationClips = false;
    ...
    FString MakeAnimSequenceCacheKey(const FString& NormalizedPath, const USkeletalMesh* TargetMesh, int32 AnimStackIndex) const;
    bool IsAnimSequenceUAssetPath(const FString& Path) const;
    ...
    UMaterialInterface* GetMaterialForStaticMeshSlot(const FString& SourcePath, const FString& SlotName);
    void ResolveStaticMeshMaterialSlots(const FString& SourcePath, FStaticMesh* StaticMesh);

    void ResolveSkeletalMeshMaterialSlots(const FString& SourcePath, FSkeletalMesh* SkeletalMesh);
    ...
    TMap<FString, UAnimationStateMachine*> AnimationStateMachineMap;
    TMap<FString, FFbxInspectCacheEntry> FbxInspectCache;
    ...
    TArray<FString> AnimationStateMachineFilePaths;
};
