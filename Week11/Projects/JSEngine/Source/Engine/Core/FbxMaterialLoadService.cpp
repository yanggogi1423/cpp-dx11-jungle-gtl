...
namespace
{
    // 인덱스 i에 대한 material .uasset 경로 생성. 등록 / 디스크 저장 / 디스크 로드 모두 동일 키 사용.
...
bool FFbxMaterialLoadService::Load(const FString& FbxFilePath, EMaterialShaderType ShaderType, ID3D11Device* Device)
{
    ...
    // Disk cache fallback: 이전 import에서 .uasset material을 디스크에 저장해두었으면 그것부터 로드.
    ...
    if (FAssetPathPolicy::FileExists(FirstMaterialKey))
    {
        ...
        for (int32 Index = 0; ; ++Index)
        {
            ...
            // 디스크 material asset에 ImportedName 필드가 저장돼있어서 가능.
    ...
    // FBX에서 추출한 material .uasset 저장을 위해 Asset/Material/Auto/ 디렉토리 보장.
    std::error_code Ec;
