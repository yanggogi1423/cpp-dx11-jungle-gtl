...
FMeshBuffer* FMeshBufferManager::GetSkeletalMeshBufferCPU(uint32 SkeletalMeshCompUUID, const USkeletalMesh* SkeletalMeshAsset, const TArray<FSkeletalMeshVertex>& Vertices, const TArray<uint32>& Indices, bool bNeedsUpload)
{
		...
		if (Existing.IsValid())
		{
			...
			if (bSameSourceMesh && bSameSize)
			{
				if (bNeedsUpload)
				{
					...
					FSkinningRuntimeStatCollector::RecordCPUBufferUpload(
...
FMeshBuffer* FMeshBufferManager::GetSkeletalMeshBufferGPU(const USkeletalMesh* SkeletalMeshAsset)
{
	if (!Device || !SkeletalMeshAsset) return nullptr;

	auto It = GpuSkeletalMeshBufferMap.find(SkeletalMeshAsset);
	if (It != GpuSkeletalMeshBufferMap.end() && It->second.IsValid())
	{
		return &It->second;
	}

	const auto& Vertices = SkeletalMeshAsset->GetVertices();
	const auto& Indices = SkeletalMeshAsset->GetIndices();
