                              ...
                              TArray<FString>* OutMaterialOrder)
{
    ...
    FExternalPathStaging StagedFbxPath;
    if (!StagedFbxPath.PrepareFile(FbxFilePath, "FBX material import"))
    {
        return false;
    }

    if (!OpenFbxScene(StagedFbxPath.GetLibraryPath(), Manager, Scene))
    {
        return false;
    }

    const fs::path FbxPathFs = StagedFbxPath.GetOriginalAbsolutePath();
