...
#include "Render/Resource/SkinningRenderUtils.h"
...
{  
   ...
   {  
       ...
       if (Cmd.Material)
       {
		   ...
		   BindSkinningConstantsIfNeeded(Context->DeviceContext, Context->RenderResources, Cmd);

           if (RenderBus->GetShowFlags().bBoneWeightHeatmap && Cmd.Type == ERenderCommandType::SkeletalMesh)
           {
               ID3D11BlendState* DebugBlendState = FResourceManager::Get().GetOrCreateBlendState(EBlendType::Opaque);
               ID3D11RasterizerState* DebugRasterizerState =
                   FResourceManager::Get().GetOrCreateRasterizerState(ERasterizerType::SolidBackCull);
               Context->DeviceContext->OMSetBlendState(DebugBlendState, nullptr, 0xFFFFFFFF);
