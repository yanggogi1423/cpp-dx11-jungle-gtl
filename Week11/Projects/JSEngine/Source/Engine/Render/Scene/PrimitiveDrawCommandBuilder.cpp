                                                    ...
                                                    FMeshBufferManager& MeshBufferManager) const
{
    ...
    case EPrimitiveType::EPT_SkeletalMesh:
    {
		...
		const bool bUseGPUSkinning = FProjectRenderSettings::IsGPUSkinningEnabled();
        FSkinningRuntimeStatCollector::RecordSubmittedComponent(
            static_cast<uint32>(SkeletalMesh->GetVertices().size()),
            static_cast<uint32>(SkeletalMesh->GetIndices().size()),
            static_cast<uint32>(SkeletalMesh->GetBones().size()),
            bUseGPUSkinning);

		if (bUseGPUSkinning)
		{
			MeshBuffer = MeshBufferManager.GetSkeletalMeshBufferGPU(SkeletalMesh);
		}
		else
		{
			...
			MeshBuffer = MeshBufferManager.GetSkeletalMeshBufferCPU(
        ...
        if (Sections.empty()) // fallback
        {
			...
			Cmd.bUseGPUSkinning = bUseGPUSkinning;
			Cmd.SkinningMatrices = &SkeletalMeshComp->GetSkinningMatrices();
			Cmd.SkinningMatrixCount = static_cast<uint32>(SkeletalMeshComp->GetSkinningMatrices().size());

            RenderBus.AddCommand(ERenderPass::Opaque, Cmd);
            FSkinningRuntimeStatCollector::RecordSubmittedDrawCommands(1, 1);
            return true;
        }
        ...
        FSkinningRuntimeStatCollector::RecordSubmittedDrawCommands(
            SubmittedSectionCount,
            SubmittedDrawCommandCount);
