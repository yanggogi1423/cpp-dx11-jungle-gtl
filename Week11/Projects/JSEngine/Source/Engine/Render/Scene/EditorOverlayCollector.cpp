                                                       ...
                                                       bool bIncludeEditorOnlyPrimitives) const
{
    ...
    for (UPrimitiveComponent* primitiveComponent : Actor->GetPrimitiveComponents())
    {
        ...
        else if (primitiveComponent->GetPrimitiveType() == EPrimitiveType::EPT_SkeletalMesh)
        {
			...
			BaseCmd.bUseGPUSkinning = FProjectRenderSettings::IsGPUSkinningEnabled();
			BaseCmd.SkinningMatrices = &SkeletalMeshComp->GetSkinningMatrices();
			BaseCmd.SkinningMatrixCount = static_cast<uint32>(SkeletalMeshComp->GetSkinningMatrices().size());
        }
