...
namespace
{
    ...
    bool ResolveMaterialNameToFiles(const FString& MaterialName, TArray<FString>& OutMaterialFiles)
    {
        ...
        for (const std::filesystem::directory_entry& Entry : std::filesystem::recursive_directory_iterator(MaterialRoot))
        {
            ...
            if (!FAssetPathPolicy::IsSerializedMaterialAssetPath(FPaths::ToRelativeString(Entry.path().wstring())))
            {
                continue;
            }
    ...
    bool AddFileDependency(FPackageCookContext& Context, const FString& Path, FString& OutMessage)
    {
        ...
        {
            OutMessage = "Legacy mesh path is not supported for packaging. Import it as .uasset first: " + NormalizedPath;
            return false;
        }
    ...
    bool AddKnownRuntimeDependencies(FPackageCookContext& Context, FString& OutMessage)
    {
        ...
        {
            "Asset/Material/DecalMat.uasset",
            "Asset/Mesh/Dice/Dice.uasset"
        };
    ...
    bool AddConfiguredPrefabDependencies(const FGameBuildSettings& Settings, FPackageCookContext& Context, FString& OutMessage)
    {
        if (Settings.DefaultPawnPrefabPath.empty())
        {
            return true;
        }

        if (!AddFileDependency(Context, Settings.DefaultPawnPrefabPath, OutMessage))
        {
            OutMessage = "Failed to include default pawn prefab dependency: " + Settings.DefaultPawnPrefabPath + " | " + OutMessage;
            return false;
        }
    ...
    bool CookAndCopyPackageAssets(const FGameBuildSettings& Settings, const std::filesystem::path& OutputRoot, const TArray<FString>& IncludedScenes, FString& OutMessage)
    {
        ...
        if (!AddRuntimeFilesByExtension(Context, "Asset/Material", { ".uasset" }, OutMessage))
        {
            return false;
        }
        if (!AddRuntimeFilesByExtension(Context, "Asset/Curve", { ".uasset" }, OutMessage))
        {
            return false;
        }
        ...
        if (!AddConfiguredPrefabDependencies(Settings, Context, OutMessage))
        {
            return false;
        }
        ...
        UE_LOG("[Build] Cook summary: Scenes=%d, Assets=%zu",
    ...
    bool CopyRuntimeAssetDirectories(const std::filesystem::path& EngineRoot, const std::filesystem::path& OutputRoot, FString& OutMessage)
    {
        constexpr const wchar_t* RuntimeAssetDirectories[] =
        {
            L"Animation",
            L"Audio",
            L"Cube",
        ...
        for (const wchar_t* DirectoryName : RuntimeAssetDirectories)
        {
            const std::filesystem::path Source = EngineRoot / L"Asset" / DirectoryName;
            const std::filesystem::path Dest = OutputRoot / L"Asset" / DirectoryName;
            if (!CopyDirectoryIfExists(Source, Dest, OutMessage))
            {
                return false;
            }
            EmitBuildLog("Copied Asset/" + FPaths::ToUtf8(std::wstring(DirectoryName)) + " directory");
        }
...
bool FGamePackager::CopyPackageFiles(const FGameBuildSettings& Settings, FString& OutMessage)
{
    ...
    if (!CopyDirectoryIfExists(EngineRoot / L"LuaDefinitions", OutputRoot / L"LuaDefinitions", OutMessage)) return false;
    EmitBuildLog("Copied LuaDefinitions directory");
    if (!CopyRuntimeAssetDirectories(EngineRoot, OutputRoot, OutMessage)) return false;
