    ...
    FSelectionManager* InSelectionManager)
{
    ...
    ViewTarget->InitDefaultComponents(ViewTarget->AddComponent<UDebugSkelMeshComponent>());
...
void FEditorViewer::ChangeTarget(const FString& InMeshFileName, const FString& InDocumentFileName)
{
    FileName = InDocumentFileName;
    USkeletalMesh* SkelMesh = FResourceManager::Get().LoadSkeletalMesh(InMeshFileName.c_str());
    if (SkelMesh && ViewTarget)
    {
        ViewTarget->GetSkeletalMeshComponent()->SetSkeletalMesh(SkelMesh);
    }
}
