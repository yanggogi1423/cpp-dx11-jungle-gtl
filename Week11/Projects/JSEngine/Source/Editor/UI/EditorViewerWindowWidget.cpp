...
void FEditorViewerWindowWidget::AddSocketOnBone(int32 BoneIdx)
{
    ...
    const FEditorSkeletalMeshSocketState BeforeState = CaptureSocketUndoState("Add Skeletal Mesh Socket");
...
void FEditorViewerWindowWidget::DeleteSocket(int32 SocketIdx)
{
    ...
    const FEditorSkeletalMeshSocketState BeforeState = CaptureSocketUndoState("Delete Skeletal Mesh Socket");
...
void FEditorViewerWindowWidget::DrawSocketInspector()
{
    ...
    const FEditorSkeletalMeshSocketState BeforeTransformState =
        CaptureSocketUndoState("Edit Skeletal Mesh Socket");
    bool bChanged = false;
    ...
    if (bChanged)
    {
        bMeshDirty = true;
        if (CachedSkComp) CachedSkComp->MarkSkinningDirty();
        RecordSocketUndoState(BeforeTransformState, "Edit Skeletal Mesh Socket");
    }
