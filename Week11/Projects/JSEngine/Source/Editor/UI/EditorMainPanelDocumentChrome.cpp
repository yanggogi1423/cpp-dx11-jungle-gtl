...
void FEditorMainPanel::RenderActiveDocumentToolbar()
{
	...
	if (ImGui::Begin("##EditorDocumentToolbar", nullptr, Flags))
	{
		...
		FEditorAnimationSequenceViewerWidget* AnimWidget = ActiveTab ? FindAnimationSequenceViewerWidgetForTab(ActiveTab->Id) : nullptr;
		FEditorViewer* Viewer = ViewerWidget ? ViewerWidget->GetViewer() : (AnimWidget ? AnimWidget->GetViewer() : nullptr);

		if (Viewer)
		{
			RenderViewerToolbarControls(Viewer, AnimWidget != nullptr);
		}
		...
		else if (ActiveTab && ActiveTab->Id.Kind == EEditorTabKind::AnimationStateMachineEditor)
		{
			ImGui::TextDisabled("Animation State Machine");
			ImGui::SameLine();
			ImGui::TextUnformatted(Widgets.AnimationStateMachineWidget.GetAssetPath().c_str());
		}
		else
		{
...
void FEditorMainPanel::RenderViewerToolbarControls(FEditorViewer* Viewer, bool bAllowViewportStats)
{
	if (!Viewer)
	{
	...
	if (ImGui::BeginPopup("##ViewerShowFlagsPopupShared"))
	{
		...
		if (bAllowViewportStats && ViewportState)
		{
			ImGui::Separator();
			ImGui::TextDisabled("Stats");
			ImGui::MenuItem("FPS", nullptr, &ViewportState->bShowStatFPS);
			ImGui::MenuItem("Skinning", nullptr, &ViewportState->bShowStatSkinning);
		}
		ImGui::EndPopup();
	}
	ImGui::PopID();
