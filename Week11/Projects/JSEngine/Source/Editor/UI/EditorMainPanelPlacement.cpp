...
FString ResolveSkeletalMeshDropLoadPath(const FString& PayloadPath)
{
    ...
    const FString AssetPath = FPaths::Normalize(FPaths::ToUtf8(RelativePath.generic_wstring()));
    FAssetMetaData MetaData;
    if (!FAssetFile::LoadMetadataOnly(AssetPath, MetaData) ||
        MetaData.ClassName != "USkeletalMesh")
    {
        return {};
    }
...
)
{
    ...
    EditorEngine->GetUndoSystem().RecordCreateActors(CreatedActors);
