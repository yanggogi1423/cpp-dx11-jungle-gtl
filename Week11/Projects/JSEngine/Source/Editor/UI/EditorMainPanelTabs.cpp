...
namespace
{
	...
	FEditorTabVisual GetTabVisual(EEditorTabKind Kind)
	{
		...
		case EEditorTabKind::AnimationSequenceViewer:
			return { ImVec4(0.93f, 0.52f, 0.24f, 1.0f), "Animation Sequence Viewer" };
        ...
        case EEditorTabKind::LuaAnimGraphEditor:
            return { ImVec4(0.52f, 0.72f, 0.95f, 1.0f), "Lua Anim Graph" };
		case EEditorTabKind::AnimationStateMachineEditor:
			return { ImVec4(0.82f, 0.48f, 0.92f, 1.0f), "Animation State Machine" };
		case EEditorTabKind::BlueprintEditor:
			return { ImVec4(0.24f, 0.68f, 0.95f, 1.0f), "Blueprint" };
		default:
...
void FEditorMainPanel::RenderEditorTabStrip()
{
	...
	if (ImGui::Begin("##EditorDocumentTabStrip", nullptr, Flags))
	{
		...
		const float AvailableTabWidth = std::max(0.0f, TabStripSize.x - HomeTabWidth - TabStripRightPadding);
		const float DynamicTabWidth = DockedTabCount > 0
			? std::clamp(AvailableTabWidth / static_cast<float>(DockedTabCount), MinDocumentTabWidth, MaxDocumentTabWidth)
			: MaxDocumentTabWidth;
		...
		for (int32 Index = 0; Index < static_cast<int32>(Tabs.size()); ++Index)
		{
			...
			const float TabWidth = DynamicTabWidth;
			...
			if (Tab.bCanClose && ImGui::BeginPopupContextItem("##TabContext"))
			{
				const bool bCanDetach =
					...
					FindAnimationSequenceViewerWidgetForTab(Tab.Id) != nullptr ||
					Tab.Id.Kind == EEditorTabKind::LuaAnimGraphEditor ||
					Tab.Id.Kind == EEditorTabKind::AnimationStateMachineEditor;
...
void FEditorMainPanel::ActivateEditorTab(const FEditorTabId& TabId)
{
	...
	else if (FEditorAnimationSequenceViewerWidget* AnimWidget = FindAnimationSequenceViewerWidgetForTab(TabId))
	{
		if (FEditorViewer* Viewer = AnimWidget->GetViewer())
		{
			EditorEngine->FocusViewportInput(&Viewer->GetViewport());
		}
	}
...
bool FEditorMainPanel::RequestCloseEditorTab(const FEditorTabId& TabId)
{
	...
	{
		CloseAnimationSequenceViewerTab(TabId);
		return true;
	}
	...
	if (TabId.Kind == EEditorTabKind::AnimationStateMachineEditor)
	{
		Widgets.AnimationStateMachineWidget.Close();
		return EditorTabs.CloseTab(TabId);
	}
...
void FEditorMainPanel::RequestDetachEditorTab(const FEditorTabId& TabId, bool bDetached)
{
	...
	FEditorAnimationSequenceViewerWidget* AnimWidget = FindAnimationSequenceViewerWidgetForTab(TabId);
	const bool bLuaAnimGraph = TabId.Kind == EEditorTabKind::LuaAnimGraphEditor &&
		MakeLuaAnimGraphEditorTabId(Widgets.LuaAnimGraphWidget.GetAssetPath()).Matches(TabId);
	const bool bAnimationStateMachine = TabId.Kind == EEditorTabKind::AnimationStateMachineEditor &&
		MakeAnimationStateMachineEditorTabId(Widgets.AnimationStateMachineWidget.GetAssetPath()).Matches(TabId);
	if (!ViewerWidget && !AnimWidget && !bLuaAnimGraph && !bAnimationStateMachine)
	{
		return;
	}
	...
	{
		AnimWidget->SetOpen(true);
		if (bDetached)
		{
			ImGui::SetWindowFocus(AnimWidget->GetWindowName().c_str());
			if (bWasActive)
			{
				const TArray<FEditorTabEntry>& Tabs = EditorTabs.GetTabs();
				if (!Tabs.empty())
	...
	{
		Widgets.LuaAnimGraphWidget.SetOpen(true);
		if (bDetached)
		{
			ImGui::SetWindowFocus(Widgets.LuaAnimGraphWidget.GetWindowName().c_str());
			if (bWasActive)
			{
				const TArray<FEditorTabEntry>& Tabs = EditorTabs.GetTabs();
				if (!Tabs.empty())
	...
	else if (bAnimationStateMachine)
	{
		Widgets.AnimationStateMachineWidget.SetOpen(true);
		if (bDetached)
		{
			ImGui::SetWindowFocus(Widgets.AnimationStateMachineWidget.GetWindowName().c_str());
			if (bWasActive)
			{
				const TArray<FEditorTabEntry>& Tabs = EditorTabs.GetTabs();
				if (!Tabs.empty())
