...
TArray<const UClass*> GetRegisteredNotifyTypes()
{
	...
	{
		if (Class && Class != UNotify::StaticClass() && Class->IsChildOf(UNotify::StaticClass()))
		{
...
{
	if (Viewer && Viewer != InViewer)
	{
		Viewer->GetClient().SetShortcutHandler(nullptr);
	}
	...
	if (Viewer)
	{
		Viewer->GetClient().SetShortcutHandler(
			[this](const FViewportInputContext& Context)
			{
				return HandleViewportShortcutInput(Context);
			});
	}
}
...
void FEditorAnimationSequenceViewerWidget::Render(float DeltaTime)
{
	...
	{
		EditorEngine->GetMainPanel().RequestDockAnimationSequenceViewer(Viewer);
		return;
	}
...
void FEditorAnimationSequenceViewerWidget::RenderDetachedDocumentToolbar(bool& bDockRequested)
{
	...
	EditorEngine->GetMainPanel().RenderViewerToolbarControls(Viewer, true);
	ImGui::EndChild();
}
...
void FEditorAnimationSequenceViewerWidget::RenderNotifyContextMenu(float FramesPerSecond, float PlayLength, int32 LastFrame)
{
	...
	if (ImGui::BeginMenu("Add Notify"))
	{
		...
		{
			ImGui::MenuItem("No UNotify classes", nullptr, false, false);
		}
		else
		{
			for (const UClass* NotifyClass : NotifyTypes)
			{
				if (!NotifyClass || NotifyClass->ClassName.empty())
				{
					continue;
				}

				if (ImGui::MenuItem(NotifyClass->ClassName.c_str()))
				{
					...
					Notify.NotifyName = FName(NotifyClass->ClassName);
...
bool FEditorAnimationSequenceViewerWidget::SaveAnimationAsset()
{
	...
	if (MetaData.ClassName != UAnimSequence::StaticClass()->ClassName)
	{
		if (EditorEngine)
		{
