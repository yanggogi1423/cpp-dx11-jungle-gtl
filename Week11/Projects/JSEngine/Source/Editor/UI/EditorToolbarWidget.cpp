...
void FEditorToolbarWidget::SetAnimationSequenceViewerOpenCallback(
	std::function<void()> InCallback,
	std::function<bool()> InCanOpenCallback)
{
	AnimationSequenceViewerOpenCallback = std::move(InCallback);
	AnimationSequenceViewerCanOpenCallback = std::move(InCanOpenCallback);
}
...
void FEditorToolbarWidget::RenderWindowMenu()
{
	...
	if (AnimationSequenceViewerOpenCallback)
	{
		const bool bCanOpenAnimSequenceViewer =
			!AnimationSequenceViewerCanOpenCallback || AnimationSequenceViewerCanOpenCallback();
		if (ImGui::MenuItem("Animation Sequence Viewer", nullptr, false, bCanOpenAnimSequenceViewer))
		{
			AnimationSequenceViewerOpenCallback();
		}
		if (!bCanOpenAnimSequenceViewer && ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
		{
			ImGui::SetTooltip("Open an Animation Sequence asset from Content Browser.");
		}
	}
...
void FEditorToolbarWidget::RenderSettingsMenu()
{
	...
	if (bShowEditorSettings)
	{
		ImGui::MenuItem("Editor Settings", nullptr, bShowEditorSettings);
	}
	if (bShowProjectSettings || bShowWorldSettings)
	{
		ImGui::Separator();
