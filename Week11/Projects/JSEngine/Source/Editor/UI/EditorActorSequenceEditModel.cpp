...
namespace
{
    ...
    FEditorObjectState PendingActorSequenceUndoBeforeState;
    FString PendingActorSequenceUndoLabel;
...
FString FEditorActorSequenceEditModel::MakeComponentLabel(AActor* Owner, UActorComponent* Component)
{
    ...
    if (Label.empty())
    {
        Label = Component->GetClass() ? Component->GetClass()->ClassName : "Component";
    }
...
{
    if (!EditorEngine || !IsSequenceComponentLive(SequenceComp))
    {
        return false;
    }

    PendingActorSequenceUndoBeforeState = EditorEngine->GetUndoSystem().CaptureObjectState(
        SequenceComp,
        UndoLabel ? UndoLabel : "Edit Actor Sequence");
    PendingActorSequenceUndoLabel = UndoLabel ? UndoLabel : "Edit Actor Sequence";
    return PendingActorSequenceUndoBeforeState.IsValid();
}
    ...
    bool bCaptureUndo)
{
    ...
    if (EditorEngine)
    {
        if (PendingActorSequenceUndoBeforeState.IsValid())
        {
            ...
            EditorEngine->GetUndoSystem().RecordObjectState(
                PendingActorSequenceUndoBeforeState,
                AfterState,
                PendingActorSequenceUndoLabel.empty()
                    ? FString("Edit Actor Sequence")
                    : PendingActorSequenceUndoLabel);
            PendingActorSequenceUndoBeforeState = FEditorObjectState();
            PendingActorSequenceUndoLabel.clear();
        }
