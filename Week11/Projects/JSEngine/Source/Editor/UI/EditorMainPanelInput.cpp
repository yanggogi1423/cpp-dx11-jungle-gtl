...
namespace
{
	bool HasImGuiWindowNameSegment(const char* WindowName, const char* Segment)
	{
		if (!WindowName || !Segment || Segment[0] == '\0')
		{
			return false;
		}

		const size_t SegmentLength = std::strlen(Segment);
		for (const char* Cursor = WindowName; *Cursor;)
		{
			const bool bAtSegmentStart = Cursor == WindowName || *(Cursor - 1) == '/';
			if (bAtSegmentStart && std::strncmp(Cursor, Segment, SegmentLength) == 0)
			{
				const char Next = Cursor[SegmentLength];
				if (Next == '\0' || Next == '/' || Next == '#' || Next == '_')
				{
					return true;
				}
			}
	...
	bool IsViewportContentWindowName(const char* WindowName)
	{
		if (!WindowName)
		{
			return false;
		...
		return
			std::strcmp(WindowName, "Viewport") == 0 ||
			std::strncmp(WindowName, "Viewport###", 11) == 0 ||
			std::strncmp(WindowName, "Viewer##", 8) == 0 ||
			HasImGuiWindowNameSegment(WindowName, "ViewportPanel") ||
			HasImGuiWindowNameSegment(WindowName, "AnimViewportHost");
	}
...
void FEditorMainPanel::Update()
{
    ...
    if (ImGuiContext* Context = ImGui::GetCurrentContext())
    {
        if (ImGuiWindow* HoveredWindow = Context->HoveredWindow)
        {
            ...
            bHoveredViewportContentWindow = IsViewportContentWindowName(HoveredName);
