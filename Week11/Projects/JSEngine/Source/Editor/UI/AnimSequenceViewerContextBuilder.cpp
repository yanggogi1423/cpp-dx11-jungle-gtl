...
bool FAnimSequenceViewerContextBuilder::BuildFromUAsset(const FString& AnimSequenceUAssetPath, FAnimSequenceViewerContext& OutContext)
{
    ...
    if (OutContext.MetaData.ClassName != UAnimSequence::StaticClass()->ClassName)
    {
	...
	OutContext.AnimSequence = FResourceManager::Get().LoadAnimSequence(AnimSequenceUAssetPath, OutContext.PreviewMesh, 0);
	if (!OutContext.AnimSequence || !OutContext.AnimSequence->HasValidData() || !OutContext.AnimSequence->GetDataModel())
	{
