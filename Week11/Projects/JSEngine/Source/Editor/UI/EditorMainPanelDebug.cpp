...
void FEditorMainPanel::RenderUndoHistoryPanel(float DeltaTime)
{
    ...
    const TArray<FEditorTransaction>& UndoEntries = UndoSystem.GetUndoHistory();
    const TArray<FEditorTransaction>& RedoEntries = UndoSystem.GetRedoHistory();
    ...
    if (ImGui::CollapsingHeader("Stat History", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        ImGui::Text("Transaction Data: %s", FormatHistoryBytes(HistoryStats.LogicalBytes).c_str());
        ...
        {
            ImGui::SetTooltip("Approx Total = transaction command memory + label capacity + entry storage.");
        }
    }
    ImGui::Separator();
    ...
    else
    {
        ...
        {
            ImGui::PushID(Index);
            const FString Label = UndoEntries[Index].Label.empty() ? FString("Editor Transaction") : UndoEntries[Index].Label;
    ...
    else
    {
        ...
        {
            const FString Label = RedoEntries[Index].Label.empty() ? FString("Editor Transaction") : RedoEntries[Index].Label;
...
void FEditorMainPanel::RenderEditorDebugPanel(float DeltaTime)
{
    ...
    if (ImGui::CollapsingHeader("BVH Maintenance", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        ImGui::SetNextItemWidth(220.0f);
        bPolicyChanged |= ImGui::SliderInt(
            "Rotation Structural Changes",
            &Settings.SpatialRotationStructuralChangeThreshold,
            1,
            256);
    ...
    {
        Widgets.ViewportOverlayWidget.RenderShadowAtlasPreview();
        Widgets.ViewportOverlayWidget.RenderShadowCubeArrayPreview();
    }
    ...
    ImGui::Separator();
    if (ImGui::Button("Save Editor Debug Settings", ImVec2(210.0f, 0.0f)))
    {
