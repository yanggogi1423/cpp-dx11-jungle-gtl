...
namespace
{
    ...
    void ApplyCurveEditorWindowClass()
    {
        ImGuiWindowClass WindowClass;
        WindowClass.ClassId = 0x4A534345u; // "JSCE" - curve editor floating window class
...
void FEditorCurveEditorWidget::OpenCurveAsset(const FString& CurvePath)
{
    ...
    AnimSequenceSaveCallback = nullptr;
    ...
    bOpenedFromAnimSequence = false;
    bPendingCurveUndo = false;
    PendingCurveUndoLabel.clear();
    PendingCurveAssetBeforeState = FEditorCurveAssetState();
    PendingCurveOwnerBeforeState = FEditorObjectState();
}
    ...
    int32 InitialSelectedKeyIndex)
{
    ...
    CurrentCurve = Curve;
    SourceSequenceComponent = nullptr;
    AnimSequenceSaveCallback = std::move(SaveCallback);
    SourceLabel = SourceLabelText;
    const int32 KeyCount = CurrentCurve ? static_cast<int32>(CurrentCurve->GetCurve().Keys.size()) : 0;
    SelectedKeyIndex = KeyCount > 0
        ? std::clamp(InitialSelectedKeyIndex >= 0 ? InitialSelectedKeyIndex : 0, 0, KeyCount - 1)
...
void FEditorCurveEditorWidget::Close()
{
    StopReferencePreview();
    CurrentPath.clear();
    SourceLabel.clear();
...
void FEditorCurveEditorWidget::Render(float DeltaTime)
{
    ...
    ApplyCurveEditorWindowClass();
    ImGui::SetNextWindowSize(ImVec2(620.0f, 520.0f), ImGuiCond_FirstUseEver);

    bool bOpen = bVisible;
    const char* WindowTitle = bOpenedFromActorSequence
        ? "Curve Editor - Actor Sequence"
        : (bOpenedFromAnimSequence ? "Curve Editor - Anim Sequence" : "Curve Editor");
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(OriginalFramePadding.x, TitleBarFramePaddingY));
    if (!ImGui::Begin(WindowTitle, &bOpen, WindowFlags))
    {
        ImGui::PopStyleVar();
        if (!bOpen)
        {
    ...
    bVisible = bOpen;
    DrawFloatingWindowChrome(bOpen, WindowTitle);
    if (!bOpen)
    {
        StopReferencePreview();
        bVisible = false;
    ...
    ImGui::SetCursorPosY(std::max(ImGui::GetCursorPosY(), FEditorChromeMetrics::ApplicationTitleBarHeight));
...
void FEditorCurveEditorWidget::DrawFloatingWindowChrome(bool& bOpen, const char* WindowTitle)
{
    ...
    {
        if (ImGui::MenuItem("Save", "Ctrl+S", false, CurrentCurve != nullptr))
        {
            SaveCurve();
        }
        if (ImGui::MenuItem("Reload", nullptr, false, !CurrentPath.empty()))
        {
            ReloadCurve();
        }
        ImGui::Separator();
    ...
    {
        if (ImGui::MenuItem("Add Key", nullptr, false, CurrentCurve != nullptr))
        {
            AddKey();
        }
        if (ImGui::MenuItem("Remove Key", nullptr, false, SelectedKeyIndex >= 0))
        {
            RemoveSelectedKey();
        }
        ImGui::EndMenu();
    ...
    {
        const bool bOpenedFromEmbeddedSource = bOpenedFromActorSequence || bOpenedFromAnimSequence;
        ImGui::TextDisabled(bOpenedFromEmbeddedSource ? "Embedded Curve" : "Curve Asset");
    ...
    const char* Title = WindowTitle ? WindowTitle : "Curve Editor";
...
void FEditorCurveEditorWidget::DrawToolbar()
{
    const bool bOpenedFromEmbeddedSource = bOpenedFromActorSequence || bOpenedFromAnimSequence;
    ImGui::Text("%s: %s",
        bOpenedFromEmbeddedSource ? "Source" : "Asset",
        CurrentPath.empty() ? (bOpenedFromEmbeddedSource ? "Embedded Curve" : "<None>") : CurrentPath.c_str());
    if (bDirty)
    {
        ImGui::SameLine();
        ImGui::TextDisabled("*");
    ...
    if (bOpenedFromAnimSequence)
    {
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(0.55f, 0.78f, 1.0f, 1.0f), "from Anim Sequence");
        if (!SourceLabel.empty())
        {
            ImGui::TextDisabled("Track: %s", SourceLabel.c_str());
        }
        ImGui::TextDisabled("Embedded curve. Saved into the owning AnimSequence binary.");
    }

    if (!bOpenedFromActorSequence)
    {
        ImGui::BeginDisabled(!CurrentCurve || (!bOpenedFromAnimSequence && CurrentPath.empty()) || !bDirty);
        ...
        ImGui::SameLine();
        ImGui::BeginDisabled(!CurrentCurve || CurrentPath.empty() || bOpenedFromAnimSequence);
...
void FEditorCurveEditorWidget::DrawCurveCanvas()
{
    ...
    {
        if (HasPendingCurveEditUndo())
        {
            CommitCurveEditUndo("Edit Curve");
        }
        ActiveKeyDragIndex = -1;
        ActiveTangentKeyIndex = -1;
        ActiveTangentHandle = -1;
...
void FEditorCurveEditorWidget::DrawKeyList()
{
    ...
    if (ImGui::BeginTable("##CurveKeys", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable))
    {
        ...
        for (int32 KeyIndex = 0; KeyIndex < static_cast<int32>(Curve.Keys.size()); ++KeyIndex)
        {
            ...
            bool bChanged = false;
            FEditorObjectState BeforeOwnerState;
            FEditorCurveAssetState BeforeAssetState;
            if (EditorEngine)
            {
                if (bOpenedFromActorSequence && SourceSequenceComponent)
                {
                    BeforeOwnerState = EditorEngine->GetUndoSystem().CaptureObjectState(
                        SourceSequenceComponent,
                        "Edit Curve Key");
                }
                else if (!bOpenedFromAnimSequence && !CurrentPath.empty())
                {
                    BeforeAssetState = EditorEngine->GetUndoSystem().CaptureCurveAssetState(
                        CurrentCurve,
                        CurrentPath,
                        "Edit Curve Key");
                }
            ...
            if (bChanged)
            {
                ...
                if (EditorEngine)
                {
                    if (BeforeOwnerState.IsValid() && SourceSequenceComponent)
                    {
                        EditorEngine->GetUndoSystem().RecordObjectState(
                            BeforeOwnerState,
                            EditorEngine->GetUndoSystem().CaptureObjectState(SourceSequenceComponent, "Edit Curve Key"),
                            "Edit Curve Key");
                    }
                    else if (BeforeAssetState.IsValid() && !CurrentPath.empty())
                    {
                        EditorEngine->GetUndoSystem().RecordCurveAssetState(
                            BeforeAssetState,
                            EditorEngine->GetUndoSystem().CaptureCurveAssetState(CurrentCurve, CurrentPath, "Edit Curve Key"),
                            "Edit Curve Key");
                    }
                }
            }
...
void FEditorCurveEditorWidget::AddKeyAt(float Time, float Value)
{
    ...
    const bool bOwnUndo = !HasPendingCurveEditUndo();
    if (bOwnUndo)
    {
        BeginCurveEditUndo("Add Curve Key");
    }
    ...
    if (bOwnUndo)
    {
        CommitCurveEditUndo("Add Curve Key");
    }
}
...
void FEditorCurveEditorWidget::RemoveKeyAtIndex(int32 KeyIndex)
{
    ...
    if (bOwnUndo)
    {
        BeginCurveEditUndo("Remove Curve Key");
    }
    ...
    if (bOwnUndo)
    {
        CommitCurveEditUndo("Remove Curve Key");
    }
}

void FEditorCurveEditorWidget::BeginCurveEditUndo(const FString& Label)
{
    if (bPendingCurveUndo || !EditorEngine || !CurrentCurve)
    {
        return;
    }

    PendingCurveUndoLabel = Label.empty() ? FString("Edit Curve") : Label;
    PendingCurveAssetBeforeState = FEditorCurveAssetState();
    PendingCurveOwnerBeforeState = FEditorObjectState();

    if (bOpenedFromActorSequence && SourceSequenceComponent)
    {
        PendingCurveOwnerBeforeState = EditorEngine->GetUndoSystem().CaptureObjectState(
            SourceSequenceComponent,
            PendingCurveUndoLabel);
        bPendingCurveUndo = PendingCurveOwnerBeforeState.IsValid();
        return;
    }

    if (!bOpenedFromAnimSequence && !CurrentPath.empty())
    {
        PendingCurveAssetBeforeState = EditorEngine->GetUndoSystem().CaptureCurveAssetState(
            CurrentCurve,
            CurrentPath,
            PendingCurveUndoLabel);
        bPendingCurveUndo = PendingCurveAssetBeforeState.IsValid();
    }
}
...
void FEditorCurveEditorWidget::CommitCurveEditUndo(const FString& Label)
{
    ...
    const FString FinalLabel = Label.empty()
        ? (PendingCurveUndoLabel.empty() ? FString("Edit Curve") : PendingCurveUndoLabel)
        : Label;

    if (PendingCurveOwnerBeforeState.IsValid() && SourceSequenceComponent)
    {
        const FEditorObjectState AfterState = EditorEngine->GetUndoSystem().CaptureObjectState(
            SourceSequenceComponent,
            FinalLabel);
        EditorEngine->GetUndoSystem().RecordObjectState(
            PendingCurveOwnerBeforeState,
            AfterState,
            FinalLabel);
    }
    else if (PendingCurveAssetBeforeState.IsValid() && !CurrentPath.empty())
    {
        const FEditorCurveAssetState AfterState = EditorEngine->GetUndoSystem().CaptureCurveAssetState(
            CurrentCurve,
            CurrentPath,
            FinalLabel);
        EditorEngine->GetUndoSystem().RecordCurveAssetState(
            PendingCurveAssetBeforeState,
            AfterState,
            FinalLabel);
    }
...
bool FEditorCurveEditorWidget::HasPendingCurveEditUndo() const
{
    return bPendingCurveUndo;
}
...
bool FEditorCurveEditorWidget::SaveCurve()
{
    if (bOpenedFromAnimSequence)
    {
        if (!CurrentCurve || !AnimSequenceSaveCallback)
        {
            return false;
        }

        if (!AnimSequenceSaveCallback(CurrentCurve))
        {
            if (EditorEngine)
            {
                EditorEngine->GetNotificationService().Error("AnimSequence curve save failed");
            }
            return false;
        }
        ...
        if (EditorEngine)
        {
            EditorEngine->GetNotificationService().Info("AnimSequence curve saved");
        }
        return true;
    }
