...
FEditorTabId MakeAnimationSequenceViewerTabId(const FString& ViewerFileName, const void* FallbackAddress)
{
    FEditorTabId TabId;
    TabId.Kind = EEditorTabKind::AnimationSequenceViewer;
    TabId.PayloadId = ViewerFileName;
    if (TabId.PayloadId.empty() && FallbackAddress)
    {
        char Buffer[64];
        snprintf(Buffer, sizeof(Buffer), "__AnimViewer_%p", FallbackAddress);
        TabId.PayloadId = Buffer;
    }
    return TabId;
}

FString MakeAnimationSequenceViewerTabLabel(const FString& ViewerFileName)
{
    if (ViewerFileName.empty())
    {
...
FEditorTabId MakeLuaAnimGraphEditorTabId(const FString& AssetPath)
{
    ...
    TabId.PayloadId = AssetPath.empty() ? "__LuaAnimGraphEditor" : AssetPath;
    return TabId;
}
...
FEditorTabId MakeAnimationStateMachineEditorTabId(const FString& AssetPath)
{
    FEditorTabId TabId;
    TabId.Kind = EEditorTabKind::AnimationStateMachineEditor;
    TabId.PayloadId = AssetPath.empty() ? "__AnimationStateMachineEditor" : AssetPath;
    return TabId;
}

FString MakeAnimationStateMachineEditorTabLabel(const FString& AssetPath)
{
    if (AssetPath.empty())
    {
...
FEditorTabId MakeBlueprintEditorTabId(const FString& AssetPath)
{
    FEditorTabId TabId;
    TabId.Kind = EEditorTabKind::BlueprintEditor;
    TabId.PayloadId = AssetPath.empty() ? "__BlueprintEditor" : AssetPath;
    return TabId;
}

FString MakeBlueprintEditorTabLabel(const FString& AssetPath)
{
    if (AssetPath.empty())
    {
        return "Blueprint";
    ...
    const size_t SlashIndex = AssetPath.find_last_of("/\\");
    const FString FileName = SlashIndex == FString::npos ? AssetPath : AssetPath.substr(SlashIndex + 1);
    return FileName.empty() ? "Blueprint" : FileName;
}
