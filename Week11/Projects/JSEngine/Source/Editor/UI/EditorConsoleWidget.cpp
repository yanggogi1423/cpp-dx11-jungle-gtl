...
#include "Engine/Render/Common/ProjectRenderSettings.h"
...
void InvalidateSkinningForWorld(UWorld* World)
{
	if (!World)
	{
		return;
...
FEditorConsoleWidget::FEditorConsoleWidget() 
{
	...
	RegisterCommand("causecrash", "Force a test crash and write a minidump.", [this](const TArray<FString>& args)
	{
		AddLog("Forcing a crash for testing purposes...");
		CauseCrash();
	});
}
...
void FEditorConsoleWidget::Render(float DeltaTime)
{
	...
	ApplyConsoleWindowClass();
	FEditorDetachedDocumentChrome::SetFirstUseWindowPlacement(ImVec2(800.0f, 600.0f), ImVec2(180.0f, 140.0f));
...
void FEditorConsoleWidget::CmdStat(const TArray<FString>& Args)
{
	...
	FEditorViewportState* ActiveAnimViewportState =
		EditorEngine->GetMainPanel().ResolveActiveAnimationSequenceViewportState();

	if (Target == "fps")
	{
		bool& bFlag = ActiveAnimViewportState
			? ActiveAnimViewportState->bShowStatFPS
			: Layout.GetViewportState(FocusedIdx).bShowStatFPS;
		bFlag = !bFlag;
		if (ActiveAnimViewportState)
		{
			AddLog("Stat FPS %s (active Animation Sequence tab)\n", bFlag ? "Enabled" : "Disabled");
		}
		else
		{
	...
	{
		bool& bFlag = ActiveAnimViewportState
			? ActiveAnimViewportState->bShowStatSkinning
			: Layout.GetViewportState(FocusedIdx).bShowStatSkinning;
		bFlag = !bFlag;
		if (ActiveAnimViewportState)
		{
			AddLog("Stat Skinning %s (active Animation Sequence tab)\n", bFlag ? "Enabled" : "Disabled");
		}
		else
		{
	...
	else if (Target == "none")
	{
		if (ActiveAnimViewportState)
		{
			ActiveAnimViewportState->bShowStatFPS = false;
			ActiveAnimViewportState->bShowStatSkinning = false;
			AddLog("Animation Sequence tab stats disabled\n");
			return;
		}
...
void FEditorConsoleWidget::CmdSkinning(const TArray<FString>& Args)
{
	...
	if (Target == "cpu")
	{
		FProjectRenderSettings::SetSkinningMode(ESkinningMode::CPU);
		FEditorSettings::Get().SaveToFile(FEditorSettings::GetDefaultSettingsPath());
		InvalidateSkinningForWorld(EditorEngine ? EditorEngine->GetFocusedWorld() : nullptr);
		AddLog("Skinning mode changed to: CPU\n");
	}
	else if (Target == "gpu")
	{
		FProjectRenderSettings::SetSkinningMode(ESkinningMode::GPU);
		FEditorSettings::Get().SaveToFile(FEditorSettings::GetDefaultSettingsPath());
		InvalidateSkinningForWorld(EditorEngine ? EditorEngine->GetFocusedWorld() : nullptr);
