...
void FEditorMainPanel::OpenRuntimeUILayoutAsset(const FString& LayoutPath)
{
    if (!LayoutPath.empty())
    {
        Widgets.RuntimeUIPreviewWidget.OpenLayoutAsset(LayoutPath);
    }

    const FString DocumentPath = Widgets.RuntimeUIPreviewWidget.GetLayoutAssetPath();
    const FEditorTabId TabId = MakeRuntimeUIPreviewTabId();
    const FString TabLabel = MakeRuntimeUIPreviewTabLabel(DocumentPath);
    EditorTabs.OpenOrFocusTab(TabId, TabLabel);
    EditorTabs.SetTabLabel(TabId, TabLabel);
