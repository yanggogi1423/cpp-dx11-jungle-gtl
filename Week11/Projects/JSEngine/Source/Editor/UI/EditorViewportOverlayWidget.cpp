...
void FEditorViewportOverlayWidget::RenderFloatingOverlays(float DeltaTime)
{
	...
	if (EditorEngine && EditorEngine->GetMainPanel().IsLevelEditorViewportVisible())
	{
...
void FEditorViewportOverlayWidget::RenderViewportSettings(float DeltaTime)
{
    ...
    ImGui::Separator();
    if (ImGui::Button("Save Viewport Settings", ImVec2(180.0f, 0.0f)))
    {
        if (FocusedState)
        {
            Settings.ViewMode = FocusedState->ViewMode;
            Settings.LightCullMode = FocusedState->LightCullMode;
        }
		...
		if (ImGui::Begin(WinId, nullptr, kFlags))
		{
			...
			if (VS.bShowStatSkinning)
			{
				...
				ImGui::TextColored(
					...
					"- Mode: %s",
					FProjectRenderSettings::IsGPUSkinningEnabled() ? "GPU" : "CPU");
				const FSkinningRuntimeStats& SkinningStats = FSkinningRuntimeStatCollector::GetFrameStats();
				ImGui::TextColored(
					ImVec4(0.7f, 0.85f, 1.0f, 1.0f),
					"- CPU Vertex: %.3f ms total / %.3f ms avg / %u calls",
					SkinningStats.TotalCPUVertexMs,
					SkinningStats.GetAvgCPUVertexMs(),
					SkinningStats.CPUVertexCallCount);
				ImGui::TextColored(
					ImVec4(0.7f, 0.85f, 1.0f, 1.0f),
					"- Pose: %.3f ms total / %.3f ms avg / %u calls",
					SkinningStats.TotalPoseUpdateMs,
					SkinningStats.GetAvgPoseUpdateMs(),
					SkinningStats.PoseUpdateCallCount);
				ImGui::TextColored(
					ImVec4(0.7f, 0.85f, 1.0f, 1.0f),
					"- Matrices: %.3f ms total / %.3f ms avg / %u calls",
					SkinningStats.TotalMatrixUpdateMs,
					SkinningStats.GetAvgMatrixUpdateMs(),
					SkinningStats.MatrixUpdateCallCount);
				ImGui::TextColored(
					ImVec4(0.7f, 0.85f, 1.0f, 1.0f),
					"- GPU Matrix Upload: %.3f ms total / %.3f ms avg / %u calls",
					SkinningStats.TotalGPUMatrixUploadMs,
					SkinningStats.GetAvgGPUMatrixUploadMs(),
					SkinningStats.GPUMatrixUploadCallCount);
				ImGui::TextColored(
					ImVec4(0.7f, 0.85f, 1.0f, 1.0f),
					"- CPU VB Upload: %.3f ms total / %.3f ms avg / %u calls",
					SkinningStats.TotalCPUBufferUploadMs,
					SkinningStats.GetAvgCPUBufferUploadMs(),
					SkinningStats.CPUBufferUploadCallCount);
				ImGui::TextColored(
					ImVec4(0.7f, 0.85f, 1.0f, 1.0f),
					"- Update: %.3f ms total / %.3f ms avg / %u calls",
					SkinningStats.TotalUpdateMs,
					SkinningStats.GetAvgUpdateMs(),
					SkinningStats.UpdateCallCount);
				ImGui::TextColored(
					ImVec4(0.7f, 0.85f, 1.0f, 1.0f),
					"- Submitted: %u comps (%u CPU / %u GPU), %u sections, %u draws",
					SkinningStats.SubmittedComponentCount,
					SkinningStats.SubmittedCPUComponentCount,
					SkinningStats.SubmittedGPUComponentCount,
					SkinningStats.SubmittedSectionCount,
					SkinningStats.SubmittedDrawCommandCount);
				ImGui::TextColored(
					ImVec4(0.7f, 0.85f, 1.0f, 1.0f),
					"- Workload: %u verts, %u indices, %u bones",
					SkinningStats.SubmittedVertexCount,
					SkinningStats.SubmittedIndexCount,
					SkinningStats.SubmittedBoneCount);
			}
...
void FEditorViewportOverlayWidget::RenderGroupedStatOverlay(float DeltaTime)
{
    ...
    if (ImGui::Begin("##Week06GroupedStatOverlay", nullptr, Flags))
    {
        ...
        ImGui::Text("Mode : %s", FProjectRenderSettings::IsGPUSkinningEnabled() ? "GPU" : "CPU");
        const FSkinningRuntimeStats& SkinningStats = FSkinningRuntimeStatCollector::GetFrameStats();
        ImGui::Text("CPU Vertex : %.3f ms total / %.3f ms avg / %u calls",
            SkinningStats.TotalCPUVertexMs,
            SkinningStats.GetAvgCPUVertexMs(),
            SkinningStats.CPUVertexCallCount);
        ImGui::Text("Pose       : %.3f ms total / %.3f ms avg / %u calls",
            SkinningStats.TotalPoseUpdateMs,
            SkinningStats.GetAvgPoseUpdateMs(),
            SkinningStats.PoseUpdateCallCount);
        ImGui::Text("Matrices   : %.3f ms total / %.3f ms avg / %u calls",
            SkinningStats.TotalMatrixUpdateMs,
            SkinningStats.GetAvgMatrixUpdateMs(),
            SkinningStats.MatrixUpdateCallCount);
        ImGui::Text("GPU Upload : %.3f ms total / %.3f ms avg / %u calls",
            SkinningStats.TotalGPUMatrixUploadMs,
            SkinningStats.GetAvgGPUMatrixUploadMs(),
            SkinningStats.GPUMatrixUploadCallCount);
        ImGui::Text("CPU VB Up  : %.3f ms total / %.3f ms avg / %u calls",
            SkinningStats.TotalCPUBufferUploadMs,
            SkinningStats.GetAvgCPUBufferUploadMs(),
            SkinningStats.CPUBufferUploadCallCount);
        ImGui::Text("Update     : %.3f ms total / %.3f ms avg / %u calls",
            SkinningStats.TotalUpdateMs,
            SkinningStats.GetAvgUpdateMs(),
            SkinningStats.UpdateCallCount);
        ImGui::Text("Submitted  : %u comps (%u CPU / %u GPU), %u sections, %u draws",
            SkinningStats.SubmittedComponentCount,
            SkinningStats.SubmittedCPUComponentCount,
            SkinningStats.SubmittedGPUComponentCount,
            SkinningStats.SubmittedSectionCount,
            SkinningStats.SubmittedDrawCommandCount);
        ImGui::Text("Workload   : %u verts, %u indices, %u bones",
            SkinningStats.SubmittedVertexCount,
            SkinningStats.SubmittedIndexCount,
            SkinningStats.SubmittedBoneCount);
