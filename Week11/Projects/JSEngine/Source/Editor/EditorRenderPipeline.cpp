...
void FEditorRenderPipeline::Execute(float DeltaTime, FRenderer& Renderer)
{
    ...
    std::chrono::steady_clock::time_point UIEnd;
    {
        ...
        // 실제로 보이는 Level Editor viewport만 렌더링합니다.
        // Asset viewer tab/window가 활성화된 상태에서는 Level viewport rect가
        // 이전 프레임 값으로 남아 있을 수 있으므로, 탭 가시성을 먼저 확인합니다.
        ViewportsStart = std::chrono::steady_clock::now();
        if (Editor->GetMainPanel().IsLevelEditorViewportVisible())
        {
            for (int32 i = 0; i < FEditorViewportLayout::MaxViewports; ++i)
            {
                const FViewportRect& Rect = Editor->GetViewportLayout().GetSceneViewport(i).GetRect();
...
void FEditorRenderPipeline::RenderViewerViewport(FRenderer& Renderer)
{
	...
	{
        const bool bShouldRenderViewer =
            Viewers[i] && Editor->GetMainPanel().ShouldRenderViewerViewport(Viewers[i].get());

        if (!bShouldRenderViewer)
        {
            continue;
        }
        ...
        ShowFlags.bBoneWeightHeatmap = VFlags.bShowBoneWeightHeatmap;
        ShowFlags.BoneWeightHeatmapBoneIndex = VFlags.bShowBoneWeightHeatmap
	...
	float Distance)
{
	...
	if (MeshBuffer == nullptr || MeshBuffer->GetIndexBuffer().GetIndexCount() == 0)
	{
		return nullptr;
	}
	...
	TArray<FMatrix> BindPoseSkinningMatrices;
	const TArray<FBoneInfo>& Bones = Mesh->GetBones();
	BindPoseSkinningMatrices.resize(Bones.size());
	for (int32 BoneIndex = 0; BoneIndex < static_cast<int32>(Bones.size()); ++BoneIndex)
	{
		BindPoseSkinningMatrices[BoneIndex] = FMatrix::Identity;
	}
	...
	auto ResolveSectionMaterial = [Mesh](int32 SectionIndex) -> UMaterialInterface*
	{
		...
		if (SectionIndex >= 0 && SectionIndex < static_cast<int32>(Slots.size()) && Slots[SectionIndex].Material)
		{
			return Slots[SectionIndex].Material;
		}
		return FResourceManager::Get().GetOrCreateMaterial("DefaultWhite", EMaterialShaderType::SurfaceLit);
	};
	...
	if (!Sections.empty())
	{
		for (int32 SectionIndex = 0; SectionIndex < static_cast<int32>(Sections.size()); ++SectionIndex)
		{
			...
			Cmd.MeshBuffer = MeshBuffer;
			Cmd.Material = ResolveSectionMaterial(SectionIndex);
			Cmd.SectionIndexStart = Section.StartIndex;
			Cmd.SectionIndexCount = Section.IndexCount;
			Cmd.PerObjectConstants = FPerObjectConstants(World, FColor::White().ToVector4());
			Cmd.WorldAABB = WorldBounds;
			Cmd.bUseGPUSkinning = !BindPoseSkinningMatrices.empty();
			Cmd.SkinningMatrices = &BindPoseSkinningMatrices;
			Cmd.SkinningMatrixCount = static_cast<uint32>(BindPoseSkinningMatrices.size());
			Bus.AddCommand(ERenderPass::Opaque, Cmd);
		}
	}
	else
