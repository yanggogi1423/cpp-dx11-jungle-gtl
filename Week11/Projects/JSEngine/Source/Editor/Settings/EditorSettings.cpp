...
void FEditorSettings::SaveToFile(const FString& Path) const
{
	...
	// Editor
	JSON EditorObj = Object();
	EditorObj[EditorKey::SkinningMode] = static_cast<int32>(FProjectRenderSettings::GetSkinningMode());
	Root[EditorKey::Editor] = EditorObj;
...
void FEditorSettings::LoadFromFile(const FString& Path)
{
	...
	// Editor
	if (Root.hasKey(EditorKey::Editor))
	{
		JSON EditorObj = Root[EditorKey::Editor];
		if (EditorObj.hasKey(EditorKey::SkinningMode))
		{
			const int32 Mode = EditorObj[EditorKey::SkinningMode].ToInt();
			FProjectRenderSettings::SetSkinningMode(
				Mode == static_cast<int32>(ESkinningMode::GPU) ? ESkinningMode::GPU : ESkinningMode::CPU);
		}
	}
