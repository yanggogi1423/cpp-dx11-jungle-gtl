...
void FEditorSceneService::MarkDirty()
{
	if (EditorEngine && !bSceneDirty)
	{
		FEditorUndoSystem& UndoSystem = EditorEngine->GetUndoSystem();
		const uint64 CurrentUndoRevision = UndoSystem.GetTransactionRevision();
		const bool bHasNewTransaction = CurrentUndoRevision != LastDirtyUndoRevision;
		const bool bIsTrackedEdit = UndoSystem.IsMutationTrackingActive() || bHasNewTransaction;
		if (!bIsTrackedEdit)
		{
			UE_LOG_WARNING("[UndoGuard] Scene marked dirty without a recent editor undo transaction.");
		}
		LastDirtyUndoRevision = CurrentUndoRevision;
	}
