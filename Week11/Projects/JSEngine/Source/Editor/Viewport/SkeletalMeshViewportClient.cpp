...
void FSkeletalMeshViewportClient::SetShortcutHandler(FShortcutHandler InHandler)
{
	ShortcutHandler = std::move(InHandler);
}

bool FSkeletalMeshViewportClient::ProcessInput(FViewportInputContext& Context)
{
	if (ShortcutHandler && ShortcutHandler(Context))
	{
		return true;
	}
