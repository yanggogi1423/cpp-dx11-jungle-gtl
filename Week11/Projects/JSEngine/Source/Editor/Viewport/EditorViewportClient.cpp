...
void FEditorViewportClient::TickEditorShortcuts(const FViewportInputContext& Context)
{
	...
	// Scene file shortcuts are handled globally by the toolbar. Handling them
	// here as well makes focused viewports dispatch the same command twice.
}
...
bool FEditorViewportClient::HandleGizmoInput(const FViewportInputContext& Context)
{
	...
	const auto ClearGizmoDragUndoCapture = [&]()
	{
		...
		GizmoDragStartComponentState = FEditorObjectState();
	...
	const auto CaptureGizmoDragUndo = [&]()
	{
		...
		if (UActorComponent* SelectedComponent = SelectionManager->GetSelectedComponent())
		{
			if (Cast<USceneComponent>(SelectedComponent))
			{
				GizmoDragStartComponentState = Editor->GetUndoSystem().CaptureObjectState(
					SelectedComponent,
					"Transform Component");
			}
		}
		...
		bGizmoDragUndoCaptured =
			GizmoDragStartComponentState.IsValid() || !GizmoDragStartActorTransforms.empty();
	};
	const auto RecordGizmoDragUndo = [&]()
	{
		...
		if (GizmoDragStartComponentState.IsValid())
		{
			if (UActorComponent* SelectedComponent = SelectionManager->GetSelectedComponent())
			{
				const FEditorObjectState EndState = Editor->GetUndoSystem().CaptureObjectState(
					SelectedComponent,
					"Transform Component");
				Editor->GetUndoSystem().RecordObjectState(
					GizmoDragStartComponentState,
					EndState,
					"Transform Component");
			}
		}
...
void FEditorViewportClient::DuplicateSelection()
{
	...
	if (Editor)
	{
		Editor->GetUndoSystem().RecordCreateActors(NewSelection);
	}
