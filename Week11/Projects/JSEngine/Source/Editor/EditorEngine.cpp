...
{
    FSkinningRuntimeStatCollector::ResetFrame();
    ...
    for (auto& ViewerPtr : Viewers)
    {
        if (ViewerPtr && MainPanel.ShouldRenderViewerViewport(ViewerPtr.get()))
        {
...
{
    auto ShouldTickViewerWorld = [this](UWorld* World) -> bool
    {
        if (!World)
        {
            return false;
        ...
        for (const auto& ViewerPtr : Viewers)
        {
            ...
            const FEditorViewportClient* ViewerClient =
                static_cast<const FEditorViewportClient*>(ViewerPtr->GetViewport().GetClient());
            if (ViewerClient && ViewerClient->GetFocusedWorld() == World)
            {
                return MainPanel.ShouldRenderViewerViewport(ViewerPtr.get());
            }
        }
        return false;
    };
    ...
    for (FWorldContext& Ctx : WorldList)
    {
        ...
        if (Ctx.WorldType == EWorldType::ViewerPreview && !ShouldTickViewerWorld(Ctx.World))
        {
            continue;
        }
...
FEditorViewer* UEditorEngine::CreateAnimationSequencePreviewViewer(
    const FString& AnimSequenceAssetPath,
    const FString& SkeletalMeshPath)
{
    if (AnimSequenceAssetPath.empty() || SkeletalMeshPath.empty())
    {
        return nullptr;
    }
    ...
    for (const auto& Viewer : Viewers)
    {
        if (Viewer && Viewer->GetFileName() == AnimSequenceAssetPath)
        {
            return Viewer.get();
        }
    }

    static int32 AnimationViewerCounter = 0;
    FString HandleStr = "__AnimSequencePreview_" + std::to_string(AnimationViewerCounter++);
    FName Handle(HandleStr.c_str());

    FWorldContext& ViewerCtx = CreateWorldContext(EWorldType::ViewerPreview, Handle, "Animation Sequence Preview");
    ApplySpatialIndexMaintenanceSettings(ViewerCtx.World);

    auto NewViewer = std::make_unique<FEditorViewer>();
    NewViewer->Init(Window, this, ViewerCtx.World, ViewerCtx.SelectionManager);
    NewViewer->ChangeTarget(SkeletalMeshPath, AnimSequenceAssetPath);
...
int32 UEditorEngine::DeleteActors(const TArray<AActor*>& Actors)
{
    ...
    // 2. Undo records actor data before the live delete mutates the world.
    UndoSystem.RecordDeleteActors(Actors);
...
{
    int32 StateViewportIndex =
        PIESession.ResolveActiveViewportIndex(ViewportLayout.GetLastFocusedViewportIndex());
    StateViewportIndex = PIESession.ResolveRegisteredViewportIndex(StateViewportIndex);

    if (const FEditorViewportClient* StateClient = ViewportLayout.GetViewportClient(StateViewportIndex))
    {
        return StateClient->GetPlayState();
    }
