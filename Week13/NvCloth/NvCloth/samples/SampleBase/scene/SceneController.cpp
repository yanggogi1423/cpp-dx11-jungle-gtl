...
void SceneController::onInitialize()
{
	...
	do
	{
		...
		mCUDAInitialized = (cuDeviceGet(&cudaDevice, 0) == CUDA_SUCCESS);
		if(!mCUDAInitialized)
			break;
#if CUDA_VERSION >= 13000
		mCUDAInitialized = cuCtxCreate(&mCUDAContext, nullptr, 0, cudaDevice) == CUDA_SUCCESS;
#else
		mCUDAInitialized = cuCtxCreate(&mCUDAContext, 0, cudaDevice) == CUDA_SUCCESS;
#endif
		if(!mCUDAInitialized)
			break;
