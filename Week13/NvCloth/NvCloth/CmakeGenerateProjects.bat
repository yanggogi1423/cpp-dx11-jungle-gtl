...
set CUDA_PATH_=
if "%USE_CUDA%"=="1" (
    call "%~dp0scripts\locate_cuda.bat" CUDA_PATH_
...
set CMAKE_COMMON_PARAMS=-DTARGET_BUILD_PLATFORM=windows -DNV_CLOTH_ENABLE_DX11=%USE_DX11% -DNV_CLOTH_ENABLE_CUDA=%USE_CUDA% -DPX_GENERATE_GPU_PROJECTS=%USE_CUDA% -DCUDA_TOOLKIT_ROOT_DIR="%CUDA_PATH_%" -DWIN8SDK_PATH="%WIN8SDK_PATH%" -DSTATIC_WINCRT=%STATIC_WINCRT%
...
echo Generating %GEN_NAME% %GEN_ARCH% %GEN_TOOLSET% in compiler\%BUILD_DIR%
if exist "compiler\%BUILD_DIR%" rmdir /s /q "compiler\%BUILD_DIR%"
mkdir "compiler\%BUILD_DIR%"
pushd "compiler\%BUILD_DIR%"
"%CMAKE%" ..\cmake\windows -G "%GEN_NAME%" -A %GEN_ARCH% -T %GEN_TOOLSET% %CMAKE_COMMON_PARAMS% -DPX_OUTPUT_DLL_DIR="%PX_OUTPUT_ROOT%bin\%OUT_DIR%" -DPX_OUTPUT_LIB_DIR="%PX_OUTPUT_ROOT%lib\%OUT_DIR%" -DPX_OUTPUT_EXE_DIR="%PX_OUTPUT_ROOT%bin\%OUT_DIR%"
...
:End
if not "%EXIT_CODE%"=="0" echo CmakeGenerateProjects.bat failed with error code %EXIT_CODE%
exit /b %EXIT_CODE%
